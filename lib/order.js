(function(){
  ...
  exports.Order = function(globals){
    ...
    this.placeOrder = function(restaurantId, tray, tip, deliveryTime, firstName, lastName, address, creditCard, user, createUser, note, callback){
      var params = [
        restaurantId
      ];
      ...
      if( note.length > 100 ) {
        callback({msg: 'Too long note. Should be less than 100 characters.'});
      }
      ...
      if (_.isString(creditCard)){
        ...
        data.card_cvc        = creditCard.card_cvc || creditCard.cvc;
        data.card_expiry     = creditCard.formatExpirationDate();
        data.card_bill_zip   = creditCard.bill_zip;
      }
