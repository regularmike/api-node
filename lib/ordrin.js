(function(){
  ...
  CreditCard = function(name, expiryMonth, expiryYear, billZip, number, cvc){
    this.name         = name;
    this.expiry_month = formatExpirationMonth(expiryMonth);
    this.expiry_year  = expiryYear;
    this.bill_zip     = billZip;
    this.card_number  = String(number);
    this.card_cvc     = cvc;
    ...
    this.validate = function(){
      ...
      // validate zip
      if (/^\d{5}$/.test(this.bill_zip) == false){
        fieldErrors.push(new FieldError("zip", "Zip must be 5 digits"));
      }
